...
bool disSort(std::pair<int, std::pair<double, double>> a, std::pair<int, std::pair<double, double>> b){ // fyx
...
std::pair<int, float> SCManager::detectLoopClosureID ( void )
{
    ...
    std::vector<std::pair<int, std::pair<double, double>>> tmp_list; // fyx
    ...
    for ( int candidate_iter_idx = 0; candidate_iter_idx < NUM_CANDIDATES_FROM_TREE; candidate_iter_idx++ )
    {
        ...
        double candidate_align = double(sc_dist_result.second);

        std::pair<int, std::pair<double, double>> tmp; // fyx
        tmp.first = candidate_indexes[candidate_iter_idx];
        tmp.second.first = candidate_dist;
        tmp.second.second = deg2rad(candidate_align * PC_UNIT_SECTORANGLE);
        tmp_list.push_back(tmp);

        if( candidate_dist < min_dist )  // 从头开始遍历找最早的最小的
        {
    ...
    std::sort(tmp_list.begin(), tmp_list.end(), disSort); // fyx
    for(int i = 0; i < reloCandidate; i++){
        candidate_list.push_back(tmp_list[i]);
    }
